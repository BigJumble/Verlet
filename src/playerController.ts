class PlayerController {
    ...
    static x: number;
    static y: number;
    static playerElement: SVGSVGElement;
    static playerSize = 40;
    static speed = 200;

    static SpawnPlayer(x: number, y: number) {
        this.x = x - this.playerSize;
        this.y = y - this.playerSize;

        this.playerElement = Assets.player(this.x, this.y, this.playerSize).root as SVGSVGElement;
        GameManager.layerMain.appendChild(this.playerElement);
    }

    static #movement(deltaTime: number) {
    ...
    static update(deltaTime: number) {
        ...
        if (this.playerElement) this.#movement(deltaTime);
        ...
        if (Actions.buttonsPressed.has(PlayerAction.OpenMenu)) {
            ...
            if (Actions.gameState.has(GameState.MenuOpen)) {
                ...
                (GameManager.instructions.elementsWithId.lmouse1 as SVGSVGElement).style.fill = "gray";
                (GameManager.instructions.elementsWithId.lmouse2 as SVGSVGElement).style.fill = "gray";
            }
            else {
                ...
                (GameManager.instructions.elementsWithId.lmouse1 as SVGSVGElement).style.fill = "";
                (GameManager.instructions.elementsWithId.lmouse2 as SVGSVGElement).style.fill = "";
            }
